...
function change_attitude_FSX(roll, pitch)
	change_attitude(roll * -57, pitch * -1.5)
end
...
function new_knob(value)
  ...
  ah_percent  = var_cap(ah_percent + value, 0, 100)
	local hoop_height = 103 + -ah_percent/100 * (103 + 95)
	-- move the image in AM
	move(img_hoop, 0, hoop_height, 512, 512)
	-- calculate value and send to XPlane
  fsx_variable_write("ATTITUDE BARS POSITION", "Percent", ah_percent)
end

---------------------------------------------
--   Controls Add                          --
---------------------------------------------
dial_knob = hw_dial_add("RPI_V2_P1_19", "RPI_V2_P1_21", new_knob)
...
fsx_variable_subscribe("ATTITUDE BARS POSITION", "Percent", new_ahref)
					  					  

					  
fsx_variable_subscribe("ATTITUDE INDICATOR BANK DEGREES", "Radians",
					   "ATTITUDE INDICATOR PITCH DEGREES", "Degrees", change_attitude_FSX)
